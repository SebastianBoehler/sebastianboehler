...
export default function CommitActivityChart({ data, activeYear }: Props) {
  ...
  // When the active year changes (driven by page scroll), scroll the chart
  // horizontally so the active bar stays in view on small screens.
  useEffect(() => {
    const container = scrollRef.current
    const activeBar = barRefs.current[activeYear]
    if (!container || !activeBar) return

    const barLeft = activeBar.offsetLeft
    const barWidth = activeBar.clientWidth
    const target = Math.max(0, barLeft - (container.clientWidth / 2 - barWidth / 2))
  ...
  return (
    ...
    >
      <div className="flex w-max md:w-full gap-2 pr-4">
        {data.map((d, i) => {
          ...
          return (
            <div
              key={d.year}
              ref={(el) => {
                barRefs.current[d.year] = el
              }}
              className="flex flex-col items-center w-10 md:w-auto md:flex-1"
            >
